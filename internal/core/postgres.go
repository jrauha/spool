...
func (s *PostgresStore) ListItemsForUser(ctx context.Context, userID, feedID string, limit, offset int) ([]Item, error) {
	if limit <= 0 || offset < 0 {
		return []Item{}, nil
	}
	rows, err := s.db.QueryContext(ctx, `
		SELECT items.id::text, items.feed_id::text, items.guid, items.url,
			items.title, items.summary, items.author, items.published_at,
			CASE
				WHEN read_override.is_read THEN read_override.read_at
				WHEN read_override.is_read = false THEN NULL
				WHEN subscriptions.read_before IS NOT NULL AND items.created_at <= subscriptions.read_before THEN subscriptions.read_before
	...
	items := make([]Item, 0, limit)
	for rows.Next() {
		item, err := scanItemWithRead(rows)
		if err != nil {
			return nil, err
		}
		items = append(items, item)
...
func (s *PostgresStore) ListLatestItemsForUser(ctx context.Context, userID string, limit, offset int) ([]Item, error) {
	if limit <= 0 || offset < 0 {
		return []Item{}, nil
	}
	rows, err := s.db.QueryContext(ctx, `
...
func scanItemWithRead(row feedScanner) (Item, error) {
	var item Item
	err := row.Scan(
		&item.ID,
		&item.FeedID,
